...
export class TreeChart {
  ...
  render() {
    ...
    //Drawing containers
    const container: any = d3.select(attrs.container);
    const containerRect: any = container.node().getBoundingClientRect();
    ...
    const width= 250;
    const height = 150;
    calc.nodeMaxWidth = d3.max(attrs.data, (d3Node) => width);
    calc.nodeMaxHeight = d3.max(attrs.data, (d3Node) => height);
   ...
   // behaviors.zoom = d3.zoom().on("zoom", (event, d) => this.zoomed(event, d)).scaleExtent(attrs.scaleExtent)
    ...
    // attrs.root = d3
    //   .stratify()
    //   .id((d: any) => {console.log('d', d);console.log('attrs', attrs); return attrs.id(d.id)})
  ...
  update({ x0, y0, x, y }) {
    ...
    const nodes = treeData.descendants().map((d) => {
      ...
      if (d.data.nodeImage && d.data.nodeImage.borderColor) {
        imageBorderColor =
          typeof d.data.nodeImage.borderColor === "object"
      ...
      if (d.data.borderColor) {
        borderColor =
          typeof d.data.borderColor === "object"
            ? this.rgbaObjToColor(d.data.borderColor)
            : d.data.borderColor;
      }
      if (d.data.backgroundColor) {
        backgroundColor = borderColor =
          typeof d.data.backgroundColor === "object"
    ...
    nodeEnter
      ...
      .style("fill", ({ _children }) => (_children ? "transparent" : "#fff"));
...
//create the tooltip
  const tooltip = d3
    ...
    const fo = nodeUpdate
      ...
      .on(
        "dragover",
        attrs.onNodeDragOver
          ? (event, { data }) => {
              attrs.onNodeDragOver(event, data);
            }
          : null
      )
      .on(
        "dragenter",
        attrs.onNodeDragEnter
          ? (event, { data }) => {
              attrs.onNodeDragEnter(event, data);
            }
          : null
      )
      .on(
        "dragleave",
        attrs.onNodeDragLeave
          ? (event, { data }) => {
              attrs.onNodeDragLeave(event, data);
            }
          : null
      )
      .on(
        ...
        attrs.onNodeDrop
          ? (event, { data }) => {
              attrs.onNodeDrop(event, data);
            }
          : null
      )
      .on("mouseover", function(event, d) {
        console.log('on mouse over', event, d)
        const[x, y] = d3.pointer(event);  
        tooltip.style("opacity", .9);
        tooltip.attr("data-date", d?.data)
        //const val = d3.format(",.1f")(d[1]);
        tooltip.html("child node" + d?.data?.nodeId + " parent" + " " + d?.data?.parentNodeId)
          .style("left", (x) + "px")
          .style("top", (y) + "px");
      })
    ...
    const nodeButtonGroups = nodeEnter
      ...
      .on("click", (d, data) => {console.log('d----',d, data); return this.onButtonClick(data)});
  ...
  restyleForeignObjectElements() {
    ...
    attrs.svg
      ...
      .style(
        "border",
        ({ data }) => `${data.borderWidth || attrs.strokeWidth}px solid`
      )
      .style("border-radius", ({ data }) => `${data.borderRadius || 0}px`)
      .style("border-color", ({ data }) => data.borderColor)
